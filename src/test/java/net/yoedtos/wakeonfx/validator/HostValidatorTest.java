...
class HostValidatorTest {
    ...
    void whenHostNameIsInvalidShouldFail() {
        Host nameInvalidHost = new Host("", PORT_NUM_ONE, null);
    ...
    void whenHostPortIsInvalidShouldFail() {
        Host portInvalidHost = new Host(SIMPLE_HOST, 0, null);
        assertThatExceptionOfType(ValidationException.class)
    ...
    void whenHostIpAddressIsInvalidShouldFail() {
        ...
        Host host = new Host(SIMPLE_HOST, PORT_NUM_ONE, ipInvalid);
        assertThatExceptionOfType(ValidationException.class)
    ...
    void whenHostMacAddressIsInvalidShouldFail() {
        ...
        Host host = new Host(SIMPLE_HOST, PORT_NUM_ONE, invalidMac);
        assertThatExceptionOfType(ValidationException.class)
                .isThrownBy(() -> hostValidator.validate(host));
    }
    ...
    void whenHostSecureOnIsInvalidShouldFail() {
        ...
        Host host = new Host(SIMPLE_HOST, PORT_NUM_ONE, invalidSecOn);
        assertThatExceptionOfType(ValidationException.class)
                .isThrownBy(() -> hostValidator.validate(host));
    }
}
